...
export function NotesPagination({ t, page, pageCount, total, perPage, onPage, onPerPage }) {
  const pages = [];
  const span = 1;
  for (let i = 0; i < pageCount; i++) {
    if (i === 0 || i === pageCount - 1 || Math.abs(i - page) <= span) pages.push(i);
    ...
    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems={{ sm: 'center' }} justifyContent="space-between" sx={{ pt: 2 }}>
      ...
      <Stack direction="row" spacing={0.5} alignItems="center">
        <IconButton size="small" disabled={page === 0} onClick={() => onPage(page - 1)} aria-label={t.nPrev}>
          <Icon d="M15 6l-6 6 6 6" size={18} sw={2} />
        </IconButton>
        {pages.map((p, i) => (p === '…'
        ...
        <IconButton size="small" disabled={page >= pageCount - 1} onClick={() => onPage(page + 1)} aria-label={t.nNext}>
          <Icon d="M9 6l6 6-6 6" size={18} sw={2} />
        </IconButton>
      </Stack>
    </Stack>
  );
