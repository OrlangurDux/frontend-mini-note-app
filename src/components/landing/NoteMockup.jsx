...
export function NoteMockup({ mode, t }) {
    ...
    <Box sx={{ position: 'relative', width: '100%', maxWidth: 460 }}>
      ...
      <Box sx={{
        ...
        <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 2 }}>
          ...
          <Box sx={{ flex: 1 }} />
          <Typography sx={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: 'text.secondary', letterSpacing: '.04em' }}>
            {t.lNoteSampleTime}
          </Typography>
        </Stack>
        ...
        <Stack spacing={1.25} sx={{ mb: 2 }}>
          {t.lNoteSampleBullets.map((line, i) => (
            <Stack key={i} direction="row" spacing={1.25} alignItems="flex-start">
              ...
              <Typography sx={{ fontSize: 14, color: 'text.primary', lineHeight: 1.55 }}>{line}</Typography>
            </Stack>
          ))}
        </Stack>

        <Box sx={{ pt: 1.5, borderTop: 1, borderColor: 'divider', fontFamily: 'JetBrains Mono, monospace', fontSize: 12, color: 'primary.main' }}>
          {t.lNoteSampleLink}
        </Box>
