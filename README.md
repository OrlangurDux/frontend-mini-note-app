# React MiniNote App
> Frontend приложения MiniNote (Mini Note) — заметки с категориями, тегами и поддержкой нескольких backend-серверов.
...
Если вы используете Claude Code, в корне репозитория можно завести локальный файл `CLAUDE.md` с описанием архитектуры проекта и частых команд для разработки — он в `.gitignore` (не хранится в репозитории), сгенерировать его заново можно командой `/init`.
