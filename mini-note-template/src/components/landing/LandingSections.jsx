// src/components/landing/LandingSections.jsx — non-hero sections of the
// landing page (Features, Workflow, Stats, Quote, CTA band). Each
// component reads `t` for copy and uses MUI primitives only.
const {
  Box, Container, Stack, Typography, Button, Paper, Grid, Divider,
} = window.MUI;
...
function SectionHeading({ eyebrow, title, sub, align = 'left' }) {
  return (
    <Stack spacing={1.25} sx={{ textAlign: align, alignItems: align === 'center' ? 'center' : 'flex-start', mb: { xs: 4, md: 6 } }}>
      ...
      <Typography variant="h4" sx={{ fontWeight: 700, fontSize: { xs: 28, md: 36 }, lineHeight: 1.15, maxWidth: 720 }}>
        {title}
      </Typography>
      {sub ? (
        <Typography sx={{ color: 'text.secondary', fontSize: 16, maxWidth: 620, lineHeight: 1.55 }}>
          {sub}
        </Typography>
      ) : null}
    </Stack>
    ...
    <Paper elevation={0} sx={{
      ...
      '&:hover': {
        ...
        boxShadow: mode === 'dark'
          ...
          : '0 20px 40px rgba(25,118,210,.10)',
      },
    }}>
      <Box sx={{
      ...
      <Typography sx={{ fontWeight: 700, fontSize: 17, mb: 0.75, letterSpacing: '-0.01em' }}>
        {title}
      </Typography>
...
function FeaturesSection({ mode, t }) {
  const I = window.Icon;
  const items = [
    { icon: <I d="M13 2 4 14h7l-1 8 9-12h-7l1-8Z" sw={1.6} />, title: t.lFeat1T, body: t.lFeat1B },
    { icon: <I d="M4 6h16M4 12h12M4 18h16" sw={2} />, title: t.lFeat2T, body: t.lFeat2B },
    { icon: <I d="m21 21-4.3-4.3"><circle cx="11" cy="11" r="7" /></I>, title: t.lFeat3T, body: t.lFeat3B },
    { icon: <I d="M10 13a5 5 0 0 0 7.5.5l3-3a5 5 0 0 0-7-7l-1.5 1.5M14 11a5 5 0 0 0-7.5-.5l-3 3a5 5 0 0 0 7 7l1.5-1.5" />, title: t.lFeat4T, body: t.lFeat4B },
    { icon: <I d="M12 2 4 6v6c0 5 3.4 9.4 8 10 4.6-.6 8-5 8-10V6l-8-4ZM9 12l2 2 4-4" />, title: t.lFeat5T, body: t.lFeat5B },
    { icon: <I d="M3 12a9 9 0 1 0 18 0 9 9 0 0 0-18 0ZM3 12h18M12 3a14 14 0 0 1 0 18M12 3a14 14 0 0 0 0 18" />, title: t.lFeat6T, body: t.lFeat6B },
  ];
...
function WorkflowSection({ mode, t }) {
  const steps = [
            ...
            <Grid key={s.n} item xs={12} md={4}>
              <Stack spacing={1.5}>
                <Stack direction="row" alignItems="baseline" spacing={1.5}>
                  <Typography sx={{
                    fontFamily: 'JetBrains Mono, monospace',
                    fontSize: 13, color: 'primary.main', fontWeight: 600,
                  }}>
                    {s.n}
                  </Typography>
...
function StatsQuoteSection({ mode, t }) {
  const stats = [
        ...
        <Grid container spacing={4} alignItems="stretch">
              ...
              {stats.map((s) => (
                <Grid key={s.l} item xs={6}>
                  <Box sx={{
                    ...
                    <Typography sx={{
                      fontWeight: 700, fontSize: { xs: 28, md: 34 },
                      letterSpacing: '-0.02em', color: 'primary.main',
                      fontFamily: 'JetBrains Mono, monospace',
                    }}>
                      {s.n}
                    </Typography>
          ...
          <Grid item xs={12} md={6}>
            <Paper elevation={0} sx={{
              ...
              <Box sx={{ fontFamily: 'JetBrains Mono, monospace', color: 'primary.main', fontSize: 28, lineHeight: 1, mb: 1 }}>“</Box>
              <Typography sx={{ fontSize: { xs: 18, md: 20 }, lineHeight: 1.5, fontWeight: 500, mb: 2.5, letterSpacing: '-0.01em' }}>
                {t.lQuote}
              </Typography>
      ...
      <Container maxWidth="lg">
        <Paper elevation={0} sx={{
          ...
          <Box sx={{ position: 'relative' }}>
            <Typography variant="h4" sx={{ fontWeight: 700, fontSize: { xs: 26, md: 34 }, letterSpacing: '-0.02em', mb: 1.5 }}>
              {t.lCtaTitle}
            </Typography>
            <Typography sx={{ color: 'text.secondary', fontSize: 16, mb: 4, maxWidth: 560, mx: 'auto' }}>
              {t.lCtaSub}
            </Typography>
...
Object.assign(window, { FeaturesSection, WorkflowSection, StatsQuoteSection, CtaBand });
